...
class DynamicPositionEmbedding(keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        #print(inputs.shape,self.positional_embedding.shape)
...
class EncoderLayer(keras.layers.Layer):
    def __init__(self, d_model, rate=0.1, h=16, additional=False, max_seq=1024):
...
class Encoder(keras.layers.Layer):
    def __init__(self, num_layers, d_model, input_vocab_size, rate=0.1, max_len=None):
        ...
        #self.bar_encoding = BarPositionEmbedding(self.d_model, max_seq=max_len, bar_id=None)
    ...
    def call(self, x, bar_x, mask=None, training=False):
        weights = []
        ...
        x = self.bar_encoding(x,bar_x,self.d_model,self.max_len)
        ...
        for i in range(self.num_layers):
            ...
            weights.append(w)
        #print(x.shape,w.shape,self.num_layers)
        return x, weights  # (batch_size, input_seq_len, d_model)
    
    def bar_encoding(self,inputs,bar_id,embedding_dim,max_seq):
        embed_sinusoid_list = np.array([
            [
                [
                    m.sin(
                        bar_id[b][pos] * m.exp(-m.log(10000) * i/embedding_dim) *
                        m.exp(m.log(10000)/embedding_dim * (i % 2)) + 0.5 * m.pi * (i % 2)
                    )
                    for i in range(embedding_dim)
                ]
